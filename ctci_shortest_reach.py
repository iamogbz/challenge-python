...
class Graph:
    """
    Simple undirected OR symmetric loopless directed
    """
    INF = None
    size = None
    nodes = None
    ...
    def path(self, s, t=None):
        ...
        while queue:  # not empty
            ...
            if t:  # if there is a target
                if u == t:
                    break  # we've hit the target
                if d[u] == self.INF:
                    return -1  # minimum distance is infinity
        ...
        return d[t] if t else d  # if no target return all distances
